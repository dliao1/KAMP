...
### Summary statistics for 1 scenario
...
runtime_summary <- runtime_long %>%
  filter(
    ...
    clust == TRUE,
    abundance == 0.3,
    method %in% c("K","KAMP","KAMP_lite","Kinhom","Kperm")
  ) %>%
  group_by(method) %>%
  summarise(
    min   = min(elapsed),
...
kable(runtime_summary, digits = 3,
      caption = "Runtime summary statistics for n = 5000, clust = TRUE, abundance = 0.3, correction = trans")
```
...
p1 + coord_cartesian(ylim = c(-5, 5))

p2 + coord_cartesian(ylim = c(-5, 15))
...
ggplot(type1_df %>% filter(method != "KAMP_lite"), aes(x = factor(n), y = type1_error, color = method, group = method)) +
...
ggplot(power_df %>% filter(method != "KAMP_lite"), aes(x = factor(n), y = power, color = method, group = method)) +
  geom_hline(yintercept = alpha, linetype = "dashed") +
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0.15, position = pd) +
  geom_point(size = 2, position = pd) +
  facet_grid(correction ~ abundance) +
  ...
  theme_minimal() + coord_cartesian(ylim = c(0.3, 1.01))
```
